...
app.get('/test', async (req, res) => {
  ...
  res.send('<html><body style="font-family:Arial;text-align:center;padding:50px"><h1 style="color:#28a745">✅ Test envoyé !</h1><p>Jean Tremblay a été créé.</p><p>Le conseiller devrait recevoir un email et SMS dans quelques secondes.</p><hr><p><a href="/test?source=Auto Fiset">Tester Auto Fiset</a> | <a href="/test?source=Site web">Tester Hyundai</a></p></body></html>');
...
app.post('/webhook/activix', async (req, res) => {
  try {
    ...
    handleActivixWebhook(req.body).catch(err => console.error('Erreur webhook:', err));
  } catch (error) {
...
app.get('/approve/:token', async (req, res) => {
  ...
  try {
    ...
    res.send('<html><body style="font-family:Arial;text-align:center;padding:50px"><h1 style="color:#28a745">✅ Message envoyé!</h1></body></html>');
  } catch (error) {
...
app.get('/health', (req, res) => {
  res.json({ status: 'ok', service: 'Activ
